...
class CompetitionAdapter:
    ...
    def translate_state_to_bot(self, engine_state: dict) -> dict:
        ...
        # Calculate current bet from player states
        # Look at the highest bet among all players this round
        max_bet = 0
        for p in players:
            if p and "action" in p:
                # Try to infer bet from player action
                # This is approximate since server doesn't expose round bets directly
                action = p.get("action", "")
                if action in ["RAISE", "CALL"]:
                    # Assume some betting happened, use pot as proxy
                    max_bet = max(max_bet, pot // len([x for x in players if x]) if players else 0)

        # Fallback: estimate from pot size and phase
        if max_bet == 0 and pot > 0:
            if state.get("phase") == "PREFLOP":
                max_bet = 10  # Assume big blind
            else:
                max_bet = pot // 4  # Rough estimate
    ...
    def translate_bot_action_to_engine(self, bot_action: dict) -> dict:
        ...
        Engine expects (EXACT FORMAT):
        {"type": "act", "action": "CALL"}
        {"type": "act", "action": "CHECK"}
        {"type": "act", "action": "FOLD"}
        {"type": "act", "action": "RAISE", "amount": 50}
        """
        action = bot_action.get("action", "").upper()  # MUST BE UPPERCASE!

        engine_msg = {
            "type": "act",  # Changed from "action" to "act"
            "action": action
        }
